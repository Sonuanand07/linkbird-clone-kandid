...
import { useInfiniteLeads, useLeadStats } from '@/lib/queries';
import { useAppStore } from '@/stores/appStore';
import { Lead } from '@/types';
import { Search, Filter, Plus, Loader2 } from 'lucide-react';
import { useInView } from 'react-intersection-observer';

const LeadsWithInfiniteScroll = () => {
  const {
    leadsSearchTerm,
    setLeadsSearchTerm,
    leadsStatusFilter,
  ...
  const { 
    ...
    isLoading,
    error 
  } = useInfiniteLeads(leadsSearchTerm, leadsStatusFilter);
  ...
  const allLeads = leadPages?.pages.flatMap(page => page.items) || [];
  const totalCount = leadPages?.pages[0]?.totalCount || 0;
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {isLoadingStats ? (
          ...
          <>
            ...
            <Card className="shadow-soft">
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-secondary-foreground">{stats?.pending || 0}</div>
                <p className="text-xs text-muted-foreground">Pending</p>
              </CardContent>
            </Card>
            <Card className="shadow-soft">
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-warning">{stats?.contacted || 0}</div>
                <p className="text-xs text-muted-foreground">Contacted</p>
              </CardContent>
            </Card>
            <Card className="shadow-soft">
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-primary">{stats?.responded || 0}</div>
                <p className="text-xs text-muted-foreground">Responded</p>
              </CardContent>
            </Card>
            <Card className="shadow-soft">
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-success">{stats?.converted || 0}</div>
...
export default LeadsWithInfiniteScroll;